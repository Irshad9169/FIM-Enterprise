...
RT_EMAIL      = "rtqueue@tickets.int.untd.com"
FIM_EMAIL_DOMAIN = "fim.int.untd.com"
...
class TicketLinkerService:
    ...
    async def post_review_to_rt(ticket_id: str, subject: str,
        ...
        """
        Post a comment to RT by sending an email via sendmail.
        RT matches the ticket by the [RT #<id>] tag in the subject line.
        The From address is derived from the JWT token username.
        """
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(
                    ...
                    f"from={from_addr} sendmail OK"
                )
                return True
            else:
                logger.error(
                    f"post_review_to_rt FAILED ticket={ticket_id} "
