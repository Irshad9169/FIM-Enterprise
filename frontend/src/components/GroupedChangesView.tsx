...
function BucketSection({ changes, changeType }: { changes: ReportChangeDetail[]; changeType: string }) {
  ...
  const style = CHANGE_KIND_STYLE[changeType] || { label: changeType, color: "text-muted-foreground" };

  return (
    <div className="px-4 pt-2 pb-2 border-t border-border/60 first:border-t-0">
      ...
      {buckets.map(b => (
        <div key={b.category} className="pb-2 last:pb-0">
          <div className="text-[13px] text-muted-foreground py-0.5 pl-1">
            <b className="text-foreground font-mono">{b.count}</b>{" "}
          ...
          <div className="font-mono text-xs text-pink-400/80 pl-4 space-y-0.5">
            ...
            {b.moreCount > 0 && (
              <div className="text-muted-foreground italic font-sans text-[11px]">
      ...
      {rollups.length > 0 && (
        <div className="font-mono text-xs text-muted-foreground pl-1 pt-0.5 pb-1 space-y-0.5">
          {rollups.map(r => (
            <div key={r.directory}>
              In <span className="text-pink-400">{r.directory}</span>,{" "}
              <b className="text-foreground">{r.count}</b> files {verbFor(r.count, changeType)}
            </div>
          ))}
        </div>
      )}
...
export function GroupedChangesView({ changes: rawChanges }: { changes: ReportChangeDetail[] }) {
  ...
  if (changes.length === 0) {
    return <div className="text-muted-foreground text-xs italic py-4 text-center">No changes.</div>;
  }
  ...
  return (
    <div>
      ...
      {details.length > 0 && (
        <div className="px-4 py-3 border-t border-border/60 bg-background/40">
          <div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center justify-between gap-2 flex-wrap">
            <span>Detailed Changes — config files</span>
            <span className="text-[10px] font-normal normal-case text-muted-foreground italic">
              hash still recorded per file, hidden here for readability
            </span>
          </div>
          <div className="space-y-1.5">
            {details.map(c => (
              <div key={c.id} className="font-mono text-xs border-t border-border/40 pt-1.5 first:border-t-0 first:pt-0">
                ...
                {c.content_diff && (
                  <pre className="mt-1 ml-4 p-2 bg-background/60 border border-border/60 rounded text-[11px] text-foreground/90 overflow-x-auto whitespace-pre">
                    {c.content_diff}
                  </pre>
                )}
              </div>
