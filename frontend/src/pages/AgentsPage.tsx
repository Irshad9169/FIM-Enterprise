...
function ConfigEditorModal({ agentId, hostname, onClose }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-card border border-input rounded-lg w-full max-w-2xl p-4" onClick={e => e.stopPropagation()}>
        ...
        {loading || !rows ? (
          <div className="text-center text-muted-foreground py-6 text-sm">Loading current config...</div>
        ) : (
            ...
            {rows.map((r, i) => (
              <div key={i} className="flex gap-2 items-start">
                <input value={r.path} onChange={e => updateRow(i, "path", e.target.value)}
                  ...
                  className="flex-1 px-2 py-1.5 bg-background border border-input rounded text-xs text-foreground outline-none font-mono" />
                ...
                <button onClick={() => setRows(rows.filter((_, idx) => idx !== i))}
                  className="text-muted-foreground hover:text-red-400 p-1.5"><X size={14} /></button>
              </div>
            ))}
        ...
        <div className="flex justify-end gap-2 mt-4 pt-3 border-t border-border">
          <button onClick={onClose} className="px-3 py-1.5 text-xs rounded bg-muted hover:bg-secondary/80">Cancel</button>
          <button onClick={handleSave} disabled={saving || loading}
            className="px-3 py-1.5 text-xs rounded bg-sky-600 hover:bg-sky-500 disabled:bg-secondary disabled:text-muted-foreground">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={exportToCSV} className="px-4 py-2 bg-muted hover:bg-secondary/80 rounded text-sm border border-input">📥 Export CSV</button>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <table className="min-w-full text-sm">
          <thead className="bg-muted text-foreground/90">
            <tr>
              <th className="px-3 py-2 text-left cursor-pointer hover:bg-secondary/80" onClick={() => handleSort("hostname")}>Hostname <SortIcon field="hostname" /></th>
              <th className="px-3 py-2 text-left cursor-pointer hover:bg-secondary/80" onClick={() => handleSort("ip_address")}>IP <SortIcon field="ip_address" /></th>
              <th className="px-3 py-2 cursor-pointer hover:bg-secondary/80" onClick={() => handleSort("status")}>Status <SortIcon field="status" /></th>
          ...
          <tbody>
                      ...
                      {editingTags === a.id ? (
                        <form onSubmit={e => { e.preventDefault(); handleSaveTags(a.id, tags); }} className="inline-flex">
                          <input value={tagInput} onChange={e => setTagInput(e.target.value)} placeholder="add tag"
                            className="w-20 px-1.5 py-0.5 bg-background border border-input rounded text-[10px] text-foreground outline-none"
                            autoFocus onBlur={() => { if (!tagInput) setEditingTags(null); }} />
                        </form>
                      ) : (
                        <button onClick={() => { setEditingTags(a.id); setTagInput(""); }}
                          className="text-muted-foreground hover:text-sky-400" title="Add tag">
                          <Tag size={12} />
                        </button>
                      )}
                    </div>
                  </td>
                  <td className="px-3 py-2 text-foreground/90 text-xs text-center">{a.last_heartbeat ? new Date(a.last_heartbeat).toLocaleString() : "-"}</td>
                  <td className="px-3 py-2 text-center">
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <div className="flex items-center justify-center gap-1.5">
                        <button onClick={() => handleScan(a.id)} disabled={scanningAgent === a.id}
                          className="px-3 py-1 bg-sky-600 hover:bg-sky-500 disabled:bg-secondary disabled:text-muted-foreground rounded text-xs font-medium transition-colors">
                        ...
                        {a.scan_pause_requested ? (
                          ...
                          <button onClick={() => pauseMutation.mutate(a.id)} disabled={pauseMutation.isPending}
                            title="Pause scan" className="p-1.5 rounded bg-muted hover:bg-secondary/80 text-foreground/90">
                            <Pause size={13} />
                          </button>
                        ) : null}
                        <button onClick={() => setConfigEditorAgent({ id: a.id, hostname: a.hostname })}
                          title="Edit monitored paths"
                          className="p-1.5 rounded bg-muted hover:bg-secondary/80 text-foreground/90">
                          <Settings size={13} />
                        </button>
                      </div>
                    </div>
            ...
            {sortedAgents.length === 0 && <tr><td colSpan={6} className="px-3 py-4 text-center text-muted-foreground">No agents registered.</td></tr>}
          </tbody>
        </table>
      </div>
