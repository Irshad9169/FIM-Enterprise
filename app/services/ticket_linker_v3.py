...
class TicketLinkerService:
    ...
    async def post_review_to_rt(ticket_id: str, content: str, token: str) -> bool:
        """
        Post a comment to an RT ticket using REST API v1 format.
        RT requires the 'content' field to contain the full message in
        RT text format: 'Action: comment\\nText: <body>'
        Both HTTP and HTTPS are tried since internal RT may differ per operation.
        """
        # RT REST API v1 comment format — content must include Action and Text
        rt_content = f"Action: comment\nText: {content.replace(chr(10), chr(10) + ' ')}"

        # Try HTTP first (same as lookup), fall back to HTTPS
        for base_url in [RT_LOOKUP_URL, RT_UPDATE_URL]:
            url = f"{base_url}/ticket/{ticket_id}/comment"
            try:
                async with httpx.AsyncClient(**HTTPX_OPTS) as client:
                    ...
                    logger.info(f"post_review_to_rt({ticket_id}) via {base_url}: status={resp.status_code} body={resp.text[:200]}")
                    if resp.status_code == 200 and "error" not in resp.text.lower():
                        return True
            except Exception as e:
                logger.error(f"post_review_to_rt({ticket_id}) via {base_url}: {e}")
