...
class ComplianceReportService:
    ...
    @staticmethod
    def generate_sox_report(data: dict) -> bytes:
        """
        Generate SOX IT General Controls (Change Management) report.
        Same structure/data shape as generate_pci_dss_report — only the
        control-summary section and framing text differ, since SOX doesn't
        map to one numbered clause the way PCI-DSS 11.5 does.
        """
        buf = io.BytesIO()
        doc = SimpleDocTemplate(buf, pagesize=letter, topMargin=40, bottomMargin=40,
                               leftMargin=50, rightMargin=50)
        ...
        # Header
        elements.append(Paragraph("SOX Compliance Report", title_style))
        elements.append(Paragraph("IT General Controls — Change Management", styles["Heading3"]))
        elements.append(Spacer(1, 10))
        ...
        # Section 1: Control Summary
        elements.append(Paragraph("1. IT General Controls Summary", h2))
        elements.append(Paragraph(
            "Sarbanes-Oxley Section 404 requires effective internal controls over "
            "systems affecting financial reporting, including IT general controls for "
            "change management — detecting unauthorized changes, maintaining an audit "
            "trail, and ensuring changes are reviewed before being treated as approved.",
            normal
        ))
        elements.append(Spacer(1, 10))
        ...
        elements.append(Paragraph(
            f"This report was automatically generated by FIM Enterprise on "
            f"{datetime.now().strftime('%Y-%m-%d %H:%M IST')}. "
            f"It provides evidence of IT general controls for change management "
            f"relevant to SOX Section 404.",
            small
        ))
